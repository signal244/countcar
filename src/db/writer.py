...
CHECKPOINT_EXTRA = '{"checkpoint":true}'
...
class TrackTrajDBWriter:
    ...
    def _to_row(
        ...
        *,
        ...
        if checkpoint:
            record["extra"] = CHECKPOINT_EXTRA
        return [record.get(field) for field in TRACK_TRAJ_FIELDS]

    @staticmethod
    def _identity(row: Iterable[Any]) -> tuple[Any, Any, Any]:
        values = list(row)
        return values[1], values[2], values[3]

    def add_track(self, record: Dict[str, Any], points: List[List[float]]) -> None:
        row = self._to_row(record, points)
        self._buffer.append(row)
        if len(self._buffer) >= self.buffer_size:
            self.flush()

    def checkpoint_track(self, record: Dict[str, Any], points: List[List[float]]) -> None:
        """Persist the latest state of an active track so a crash loses less work."""
        if not points:
            return
