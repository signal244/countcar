...
def _segment_hit(p_from: Tuple[float, float], p_to: Tuple[float, float], lines: List[Dict]) -> Optional[str]:
    for ln in lines:
        pts = ln.get("points", [])
        if len(pts) >= 2 and segment_intersects(p_from, p_to, tuple(pts[0]), tuple(pts[1])):
...
def extrapolate_line_hits_detailed(
    ...
    norm_lines: List[Tuple[str, str, Tuple[float, float], Tuple[float, float], Tuple[float, float]]],
    horizon: float,
    ...
    def _hit(seg_a: Tuple[float, float], seg_b: Tuple[float, float], vdir: Tuple[float, float]) -> Optional[Tuple[str, str, str]]:
        ...
        for line_id, bound, b1, b2, n_in in norm_lines:
            if segment_intersects(seg_a, seg_b, b1, b2):
                nx, ny = n_in
                s = vx * nx + vy * ny
                if s > 0:
                    io = "in"
    ...
    head_event = None
    if u0 is not None:
        a = (float(p0[0] - u0[0] * horizon), float(p0[1] - u0[1] * horizon))
    ...
    if u1 is not None:
        ...
        b = (float(p_last[0] + u1[0] * horizon), float(p_last[1] + u1[1] * horizon))
...
def _normalize_lines(lines: List[Dict]) -> List[Tuple[str, Tuple[float, float], Tuple[float, float]]]:
    out = []
    for ln in lines:
...
def _bound_in_dir(bound: str) -> Tuple[float, float]:
    b = str(bound or "").strip().lower()
    # 화면 좌표계: x→오른쪽(+), y→아래(+)
    if b == "north_bound":
        return (0.0, 1.0)  # 위→아래 = in
    if b == "south_bound":
        return (0.0, -1.0)  # 아래→위 = in
    if b == "west_bound":
        return (1.0, 0.0)  # 좌→우 = in
    if b == "east_bound":
        return (-1.0, 0.0)  # 우→좌 = in (좌→우는 out)
    return (0.0, 0.0)


def _normalize_lines_detailed(
    lines: List[Dict],
) -> List[Tuple[str, str, Tuple[float, float], Tuple[float, float], Tuple[float, float]]]:
    ...
    """
    out: List[Tuple[str, str, Tuple[float, float], Tuple[float, float], Tuple[float, float]]] = []
    for ln in lines or []:
        pts = ln.get("points") or []
        if len(pts) < 2:
            continue
        ...
        if (inx, iny) != (0.0, 0.0):
            # bound가 정의된 경우: 라인 방향/점 순서(p1->p2)와 무관하게 'in' 방향을 고정
            nx, ny = float(inx), float(iny)
        else:
            # 알 수 없는 bound는 기존 방식(라인의 왼쪽 법선)으로 fallback
            dx = float(p2[0] - p1[0])
            dy = float(p2[1] - p1[1])
            nx, ny = (-dy, dx)
        ...
        out.append((line_id, bound, p1, p2, (nx, ny)))
    return out


def _cross_events_from_pts(
    pts_raw: List[List[float]],
    norm_lines: List[Tuple[str, Tuple[float, float], Tuple[float, float]]],
        ...
        for line_id, bound, b1, b2, n_in in norm_lines:
            if segment_intersects(a1, a2, b1, b2):
                ...
                if s > 0:
                    ...
                    inout = "unk"
                events.append((float(tms2) / 1000.0, str(line_id), str(bound), inout))
...
def count_track_trajs_streaming(
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        loaded = 0
        for tid, cls_name, blob in conn.execute(sql, tuple(params)):
            tid_s = str(tid)
            ...
            cls_by_tid[tid_s] = str(cls_name or "")
            try:
                _fid0, t0, x0, y0 = pts_raw[0]
                _fidH, tH, xH, yH = pts_raw[1]
            ...
            try:
                p0 = pts_raw[0]
                pH = pts_raw[1]
                head_xy_by_tid[tid_s] = [(float(p0[2]), float(p0[3])), (float(pH[2]), float(pH[3]))]
            except Exception:
                head_xy_by_tid[tid_s] = []

            try:
                p1 = pts_raw[-2]
                p2 = pts_raw[-1]
                tail_xy_by_tid[tid_s] = [(float(p1[2]), float(p1[3])), (float(p2[2]), float(p2[3]))]
            except Exception:
                tail_xy_by_tid[tid_s] = []

            if mode_norm in ("approach", "ap"):
                events_by_tid[tid_s] = _cross_events_detailed_from_pts(pts_raw, norm_lines_d)
            else:
                events_by_tid[tid_s] = _cross_events_from_pts(pts_raw, norm_lines)

    log(f"[count] loaded track_trajs tracks={len(events_by_tid)}")
...
def _count_approach_from_events(
    ...
    """접근로 카운트: 트랙별 첫/마지막 교차만 사용해 line_id_bound_in/out 키로 집계."""
    rows: List[Dict] = []
    for tid, evts in events_by_tid.items():
        if not evts:
            continue
        evts_sorted = sorted(evts, key=lambda x: x[0])
        pick: List[Tuple[float, str, str, str]]
        if len(evts_sorted) >= 2:
            pick = [evts_sorted[0], evts_sorted[-1]]
        else:
            pick = [evts_sorted[0]]
        for ts, line_id, bound, inout in pick:
...
def run_count(
    ...
    if _has_track_trajs(Path(db_path)):
        counts_multi, counts_final = count_track_trajs_streaming(
    ...
    def _write_excel(path: Path) -> None:
        cls_map = {
            "car": "승용차",
            # 현재 파이프라인 분류에서는 bus/truck만 들어올 수 있어도, 포맷은 세분화 컬럼을 고정 유지
            "bus": "버스대형",
            "truck": "대형트럭",
        }
        fixed_cls_cols = ["승용차", "버스소형", "버스대형", "소형트럭", "중형트럭", "대형트럭"]

        # 커스텀 8클래스 -> 엑셀 집계(6그룹)로 표시명 통일
        cls_map = {
            "passenger_car": "승용차",
            "small_bus": "소형버스",
            "large_bus": "대형버스",
        ...
        fixed_cls_cols = ["승용차", "소형버스", "대형버스", "소형화물", "중형화물", "대형화물"]
        ...
        for line_from, line_to in directions:
            ...
            if part.empty:
                pivot = pd.DataFrame(index=slots_all, columns=fixed_cls_cols).fillna(0)
            else:
                pivot = (
