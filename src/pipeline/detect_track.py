...
def _add_class_evidence(
    ...
    """Accumulate per-frame evidence instead of trusting one peak detection."""
    evidence = meta.setdefault("_class_evidence", {})
    key = f"{class_id}\u0000{class_name}\u0000{vehicle_type}"
    ...
    conf = max(0.01, float(confidence))
    item["score"] = float(item["score"]) + conf
    item["count"] = int(item["count"]) + 1
    item["max_confidence"] = max(float(item["max_confidence"]), float(confidence))


def _apply_stable_class(meta: Dict) -> None:
    evidence = meta.get("_class_evidence") or {}
    if not evidence:
        return
    best = max(
        evidence.values(),
    ...
    meta["class_id"] = int(best.get("class_id", -1))
    meta["class_name"] = str(best.get("class_name") or "")
    meta["vehicle_type"] = str(best.get("vehicle_type") or meta["class_name"])
    meta["confidence"] = float(best.get("max_confidence") or 0.0)
...
class DetectionTracker:
    ...
    def _build_track_record(
        ...
        if meta.get("end_frame") is None:
            meta["end_frame"] = int(src_frame_id)
        if meta.get("end_ts_ms") is None:
            meta["end_ts_ms"] = int(timestamp_ms)
        if meta.get("track_len") is None:
            meta["track_len"] = int(state.length)
        if meta.get("entry_x") is None:
            meta["entry_x"] = float(state.entry[0])
        if meta.get("entry_y") is None:
            meta["entry_y"] = float(state.entry[1])
        if meta.get("exit_x") is None:
            meta["exit_x"] = float(state.last_center[0])
        if meta.get("exit_y") is None:
            meta["exit_y"] = float(state.last_center[1])
    ...
    def _checkpoint_active(
        ...
        for track_id, state in active.items():
            points = traj_buf.get(track_id) or []
            if not points:
                continue
            record = self._build_track_record(
                track_id,
                state,
                frame_id,
                session_id,
                frame_duration_ms,
                vid_stride,
                meta_buf.get(track_id),
            )
            db_writer.checkpoint_track(record, sorted(points, key=lambda row: row[0]))
    ...
    def _write_end_record(
        ...
        db_writer.add_track(record, points)
